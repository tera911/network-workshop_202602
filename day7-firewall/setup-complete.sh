...
echo ""
echo "  # 外部 → 内部（拒否）"
echo "  docker exec clab-${LAB_NAME}-host-external ping -c 3 -W 2 192.168.1.10"
echo ""
echo "  # 外部 → DMZ HTTP テスト"
echo "  # ターミナル1: docker exec -it clab-${LAB_NAME}-host-dmz sh -c 'while true; do echo -e \"HTTP/1.0 200 OK\r\n\r\nHello from DMZ\" | nc -l -p 80; done'"
echo "  # ターミナル2: docker exec clab-${LAB_NAME}-host-external sh -c 'echo \"GET /\" | nc -w 3 172.16.0.10 80'"
